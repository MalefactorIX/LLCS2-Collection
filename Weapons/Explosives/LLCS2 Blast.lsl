//This is an impact-explosive with LBA support.
vector vel;//used to prevent object from passing through walls, ignore.
vector color=<1.0,0.5,0.0>;//Color used by particle system. Change as desired.
boom(vector pos, integer timed)
{
    ...
    llParticleSystem([
        ...
        llTriggerSound("01729e19-162b-699d-d45a-357a9d5e3656",1.0);//Explosion sound, change as needed
}
integer dmg=15;//LBA damage dealt by explosive
purge(integer hex,key targ, string name)//Function that deals said LBA damage
{
...
default
{
    ...
    {
        vel=llGetVel();
        if(p)llSetTimerEvent(0.1);//Used to constantly update velocity for passthrough protection
    }
    ...
    sensor(integer d)
    {
        while(d--)
        {
            ...
            if(llList2Vector(ray,1)==ZERO_VECTOR)llDamage(hit,100.0,102);//use Force (6) or Blast (102) damage types for explosives.
        }
